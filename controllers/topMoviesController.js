import topPlayingModel from "../models/topMovieSchema.js"
...
const addMovies = async (req, res) => {
    try {
        ...
        const moviesDate = new topPlayingModel(movieDetails);
...
const editMovies = async (req, res) => {
    try {
        const { id } = req.params;
        const editMovies = await topPlayingModel.findByIdAndUpdate(id, req.body, { new: true });
...
const deleteMovies = async (req, res) => {
    try {
        const { id } = req.params;
        await topPlayingModel.findByIdAndDelete(id);
...
const getMovies = async (req, res) => {
    try {
        const allMovies = await topPlayingModel.find()
...
const getMoviesById = async (req, res) => {
    try {
        ...
        const singleMovie = await topPlayingModel.findById(id);
        if (!singleMovie) {
