import nowPlayingModel from "../models/nowPlayingSchema.js"
...
const addMovies = async (req, res) => {
    try {
        ...
        const moviesDate = new nowPlayingModel(movieDetails);
...
const editMovies = async (req, res) => {
    try {
        const { id } = req.params;
        const editMovies = await nowPlayingModel.findByIdAndUpdate(id, req.body, { new: true });
...
const deleteMovies = async (req, res) => {
    try {
        const { id } = req.params;
        await nowPlayingModel.findByIdAndDelete(id);
...
const getMovies = async (req, res) => {
    try {
        const allMovies = await nowPlayingModel.find()
...
const getMoviesById = async (req, res) => {
    try {
        ...
        const singleMovie = await nowPlayingModel.findById(id);
        if (!singleMovie) {
